...
> 마지막 갱신: **2026-08-10(진단 추론 로직 src/dl/infer.py 흡수 PR #60 — FastAPI 서빙 분리(이슈 #59) PR 1/4)** · 레포 [github.com/luma200ok/smartfarm_ai](https://github.com/luma200ok/smartfarm_ai) (branch `main`)
...
### 🔥 활성
- [ ] **FastAPI 모델 서빙 API 분리**(이슈 #59, 진행 중 — PR 1 ✅): PR 2 `api/` 신설(`/api/health`·`/api/diagnoses`·`/api/prescriptions`, uvicorn workers=1) → PR 3 Streamlit `SMARTFARM_API_URL` 경유 전환(미설정 시 in-process 폴백) → PR 4 배포(`smartfarm-api.service`+Caddy `/api/*`+deploy.sh). 관제/대시보드/forecast API화는 범위 제외(스테이트풀·실익 낮음). handoff: `docs/_local/handoff/api-next.md`
