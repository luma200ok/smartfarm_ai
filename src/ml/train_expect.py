...
# "습도" 타깃(이슈 #37) — 온도(평균/최저)와 동일 파이프라인(피처·GKF·트리밍)으로 실내
# 습도(습도내부_평균)를 함께 학습한다. env_daily.csv에 외기 습도 컬럼이 없어 피처는
# 온도 타깃과 동일(외기 온도·일사량·계절)하게 유지 — 있으면 추후 피처 추가 검토 대상.
TARGETS = {"평균": "온도내부_평균", "최저": "온도내부_최저", "습도": "습도내부_평균"}
