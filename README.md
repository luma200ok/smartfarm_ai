...
- ✅ **오늘 운영 모드(이슈 #23)** — 리플레이가 아니라 **오늘 날짜로 도는 농장**: 기상청 실황·시간별 예보 → 기대값 모델로 오늘 내부 온·습도 예측 → 밴드 초과 시간대 장치 자동 ON → **제어 후 온도가 밴드 안으로 조정**되는 타임라인 차트. 서버 systemd 타이머(매시)가 장치 전환·이상·긴급을 **디스코드로 상주 발송**(파일 dedup). 기존 리플레이·시나리오는 [시뮬레이션] 탭.
